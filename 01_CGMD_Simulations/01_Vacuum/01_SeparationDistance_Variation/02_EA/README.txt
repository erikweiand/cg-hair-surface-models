In order to run a simulation, simply copy the following files from "runfiles" into the desired subdirectory:
	...
	4) system.in.settings - Force field settings/atom groups (different from 18-MEA settings!!!)

Each subdirectory already includes the initial configuration topology consisting of fully-functionalized 
surfaces. No charged beads exist for the separation distance studies. Simulations should be run using LAMMPS:

	lmp -i run.in.min
	lmp -i run.in.nvt

Paper results were obtained using parallel runs with 8 to 48 cores on a single node each.
